...
import { application, photo, skinapplication } from '../data-type';
...
export class SkinComponent {
  ...
  public handleImage(webcamImage: WebcamImage): void {
    ...
    this.http.post<photo>(' http://localhost:3000/skin', imageJSON)
      .subscribe(
        (result) => {
          console.log('Image sent to server:', result);
          if(result){
            this.addProductMessage="image is successfully added to skin care AI"
          }
  ...
  submit(data:skinapplication){
    this.product.skinapplication(data).subscribe((result)=>{
      console.warn(result);
      if(result){
