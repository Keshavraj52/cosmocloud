...
export class HairComponent {
  ...
  public handleImage(webcamImage: WebcamImage): void {
    ...
    this.http.post<photo>(' http://localhost:3000/hair', imageJSON)
      .subscribe(
        (result) => {
          console.log('Image sent to server:', result);
          if(result){
            this.addProductMessage="image is successfully added to hair care AI"
          }
  ...
  submit(data:skinapplication){
    this.product.hairapplication(data).subscribe((result)=>{
      console.warn(result);
      if(result){
